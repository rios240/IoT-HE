...
class FogNode(object):
    ...
    QUEUE = "device-unprocessed"
    HOST = "rabbit-fog.iot-he.com"
    PORT = 5671
    VIRTUAL_HOST = "/"

    def __init__(self, ca_cert, node_cert, node_key, routing_keys, seal_params, seal_relin_keys):
        ...
        self._seal_relin_keys = seal.RelinKeys()
        self._seal_relin_keys.load(self._seal_context, seal_relin_keys)
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        LOGGER.info('### Received message # %s from %s, operation x^4',
        ...
        cipher_result = self.run_seal_operation(cipher_msg)
        pickled_result = pickle.dumps(cipher_result)
        b64_pickled_result = base64.b64encode(pickled_result).decode('utf-8')

        routing_key = str(properties.headers["device_id"]) + ".processed"
        self._channel.basic_publish(exchange=self.EXCHANGE, routing_key=routing_key,
                                    body=b64_pickled_result,
                                    properties=properties)

        self.acknowledge_message(basic_deliver.delivery_tag)

    def run_seal_operation(self, cipher_msg):
        evaluator = seal.Evaluator(self._seal_context)

        msg_squared = evaluator.square(cipher_msg)
        evaluator.relinearize_inplace(msg_squared, self._seal_relin_keys)

        msg_4th = evaluator.square(msg_squared)
        evaluator.relinearize_inplace(msg_squared, self._seal_relin_keys)

        return msg_4th.to_string()
...
class ReconnectingFogNode(threading.Thread):
    def __init__(self, ca_cert, node_cert, node_key, routing_keys, seal_params, seal_relin_keys):
        super().__init__()
        ...
        self._seal_relin_keys = seal_relin_keys
        self._node = FogNode(ca_cert=self._ca_cert, node_cert=self._node_cert, node_key=self._node_key,
                             routing_keys=self._routing_keys, seal_params=self._seal_params,
                             seal_relin_keys=self._seal_relin_keys)
